...
def create_day_stub(day: int, create_test_file: bool):
    if not os.path.exists(day_src := f'src/{day}'):
        os.mkdir(day_src)
    if not os.path.exists(day_data := f'data/{day}'):
        os.mkdir(day_data)

    # Create day.py
    override_day = True
    if os.path.exists(file_name := f'src/{day}/day{day}.py'):
        override_day = _prompt_override(file_name)
    if override_day:
        with open(f'src/{day}/day{day}.py', 'w') as fh:
            ...
            fh.write("    # problem.submit(part_a(load(problem)), 'a')\n")
            fh.write("    # problem.submit(part_b(load(problem)), 'b')\n")

    # Create test.in (optional)
    if create_test_file:
        if os.path.exists(file_name := f'src/{day}/test.in'):
            if not _prompt_override(file_name):
                return
        with open(f'src/{day}/test.in', 'w'):
            pass
